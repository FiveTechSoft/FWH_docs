...
(function() {
  ...
  // Language switcher — build relative URL to same page in other language
  var pagePath = path.replace(/^.*(\/(en|es|pt)\/.*)$/, '$1');  // /FWH_docs/en/reference/fn.html → /en/reference/fn.html
  var langLinks = '<div style="padding:4px 16px 8px;font-size:11px">' +
    '<a href="../..' + pagePath.replace(/\/(en|es|pt)\//, '/en/') + '" style="color:' + (lang==='en'?'var(--text-link)':'var(--text-dim)') + ';margin-right:6px">EN</a>' +
    '<a href="../..' + pagePath.replace(/\/(en|es|pt)\//, '/es/') + '" style="color:' + (lang==='es'?'var(--text-link)':'var(--text-dim)') + ';margin-right:6px">ES</a>' +
    '<a href="../..' + pagePath.replace(/\/(en|es|pt)\//, '/pt/') + '" style="color:' + (lang==='pt'?'var(--text-link)':'var(--text-dim)') + '">PT</a>' +
    '</div>';
